...
class EventRecordNFS4(database.Model):

    __tablename__ = "event_records_nfs4"
    ...
    # General
    instance_id = database.Column(database.Integer, database.ForeignKey('instances_nfs4.id'), nullable=False)
    event_id = database.Column(database.Integer, database.ForeignKey('events_nfs4.id'), nullable=False)
    track_id = database.Column(database.Integer, database.ForeignKey('tracks_nfs4.id'), nullable=True)
    ...
    # Conditions
    is_backwards = database.Column(database.Boolean, default=False, nullable=True)
    is_mirrored = database.Column(database.Boolean, default=False, nullable=True)
    is_at_night = database.Column(database.Boolean, default=False, nullable=True)
    is_weather_on = database.Column(database.Boolean, default=False, nullable=True)
